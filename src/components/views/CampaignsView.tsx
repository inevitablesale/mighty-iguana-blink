              ...
              <TableBody>
                {campaigns.map((campaign) => (
                  <TableRow key={campaign.id}>
                    ...
                    <TableCell>
                      <Badge variant={getStatusBadgeVariant(campaign.status)} className={`${campaign.status === 'meeting' ? 'bg-accent text-accent-foreground' : ''} ${campaign.status === 'placed' ? 'bg-green-600 text-white' : ''}`}>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <ViewCampaignEmailDialog campaign={campaign}><DropdownMenuItem onSelect={(e) => e.preventDefault()}><FileText className="mr-2 h-4 w-4" />View Email</DropdownMenuItem></ViewCampaignEmailDialog>
                          {campaign.status === 'draft' && (<EditCampaignDialog campaign={campaign} onCampaignUpdated={fetchCampaigns}><DropdownMenuItem onSelect={(e) => e.preventDefault()}><Edit className="mr-2 h-4 w-4" />Edit Draft</DropdownMenuItem></EditCampaignDialog>)}
                          {['replied', 'meeting', 'sent'].includes(campaign.status) && (<GenerateProposalDialog campaign={campaign} onProposalCreated={() => { fetchCampaigns(); setCurrentView('proposals'); }}><DropdownMenuItem onSelect={(e) => e.preventDefault()}><FileText className="mr-2 h-4 w-4" />Generate Proposal</DropdownMenuItem></GenerateProposalDialog>)}
                          {['replied', 'meeting', 'sent'].includes(campaign.status) && (<CreatePlacementDialog campaign={campaign} onPlacementCreated={() => { fetchCampaigns(); setCurrentView('placements'); }}><DropdownMenuItem onSelect={(e) => e.preventDefault()}><Award className="mr-2 h-4 w-4" />Mark as Placed</DropdownMenuItem></CreatePlacementDialog>)}
                          <DropdownMenuSub>
                            <DropdownMenuSubTrigger><CheckSquare className="mr-2 h-4 w-4" /><span>Update Status</span></DropdownMenuSubTrigger>
